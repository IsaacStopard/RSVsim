...
incidence_rate_p[1:nAges,2:nVaccStates] <- lambda[i] * sigma_vect[i] * (1 - VE_INF[i,(j-1)]) * Sp[i,j]

incidence_rate_s[1:nAges,1] <- lambda[i] * sigma_vect[i] * alpha_vect[i] * Ss[i,1]
incidence_rate_s[1:nAges,2:nVaccStates] <- lambda[i] * sigma_vect[i] * (1 - VE_INF[i,(j-1)]) * alpha_vect[i] * Ss[i,j]

VE_INF <- parameter()
dim(VE_INF) <- c(nAges, nVaccStates - 1)
