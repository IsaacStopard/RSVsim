...
// [[dust2::parameter(mixing, type = "real_type", rank = 2, required = TRUE, constant = FALSE)]]
...
public:
  ...
  struct shared_state {
    ...
    struct dim_type {
      dust2::array::dimensions<2> mixing;
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    dim.mixing.set({static_cast<size_t>(nAges), static_cast<size_t>(nAges)});
    ...
    std::vector<real_type> mixing(dim.mixing.size);
    dust2::r::read_real_array(parameters, dim.mixing, mixing.data(), "mixing", true);
    ...
    return shared_state{odin, dim, nAges, b0, b1, phi, delta, gamma_s, gamma_p, nu, omega_vect, prop_detected_vect, sigma_vect, alpha_vect, mixing, Sp0, Ep0, Ip0, Ss0, Es0, Is0, R0, Incidence0, DetIncidence0};
  }
  ...
  static void update_shared(cpp11::list parameters, shared_state& shared) {
    ...
    dust2::r::read_real_array(parameters, shared.dim.mixing, shared.mixing.data(), "mixing", false);
  ...
  static void rhs(real_type time, const real_type* state, const shared_state& shared, internal_state& internal, real_type* state_deriv) {
    ...
    for (size_t i = 1; i <= shared.dim.s_ij.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.s_ij.dim[1]; ++j) {
        internal.s_ij[i - 1 + (j - 1) * shared.dim.s_ij.mult[1]] = shared.mixing[i - 1 + (j - 1) * shared.dim.mixing.mult[1]] * internal.temp[j - 1];
      }
    }
