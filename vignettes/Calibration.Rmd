...
**Running the model with the fitted parameters.** The ABC algorithms return matrices of the fitted parameter values. For `RSVsim_ABC_rejection` this is the main output and for `RSVsim_ABC_SMC` a list of the fitted parameter values is returned for each generation. The fitted parameter columns correspond to the parameter and the rows correspond to each accepted parameter combination. To run the model with one particle (combination of fitted parameters) we can therefore select the correct matrix of fitted parameters and use the `RSVsim_update_parameters` to adjust the parameters. For example, if I want to run the *first* accepted particle:
