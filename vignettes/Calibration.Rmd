...
fixed_parameter_list <- RSVsim_calibration_parameters(data = data,
                                                      fitted_parameter_names = c("phi"),
                                                      overrides = list("b0" = 0.15, "b1" = 0.25),
                                                      country = "United Kingdom",
                                                      warm_up = 365 * 3,
                                                      cohort_step_size = 10)

RSVsim_log_likelihood_dust(fitted_parameters = c(#"b0" = 0.15, 
                                            #"b1" = 0.25, 
                                            "phi" = 10),
...
parameters_fit <- RSVsim_parameters(overrides = list("b0" = 0.15, #fitted_values$par[1], 
                                                     "b1" = 0.25, #fitted_values$par[2], 
                                                     "phi" = fitted_values$par[1]),#[3]),
                                    contact_population_list = contact_population_list)
...
ggplot2::ggplot(data = data |> dplyr::mutate(age_chr = factor(data$age_chr, 
...
To fit the posterior estimates of the $b0$, $b1$ and $\phi$ parameters we use the monty package to run Hamiltonian MCMC.
