...
## Default parameters
To obtain a list of the default parameters to run the model use the [R/RSVsim_parameters.R](https://github.com/IsaacStopard/RSVsim/blob/master/R/RSVsim_parameters.R) function. The parameters can be changed by using the overrides argument; for example we override $b0$ (the transmission rate coefficient) with a value of $0.15$. The "fitted" argument refers to parameters that will be excluded - the function will not return any parameters named in this vector. 
...
The simulated number of people that are susceptible and infected, with the line type indicating whether the infection is primary or secondary is tracked in the model outputs. The incidence and detected incidence are calculated. To calculate the detected incidence age-specific detection probabilities are used, and in the default parameters we assume very few people (1%) over the age of 2 years old is tested. The age_specific prevalence is also calculated.
